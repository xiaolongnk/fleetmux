...
#   --with-fish       Opt in to Fish shell installation (changes login shell via
#                     chsh, writes ~/.config/fish/config.fish, sets tmux
...
#
# Post-install verification: the installer now fails loudly (exit 1) if any
# wiring it promised did not land — e.g. missing fish config, missing git
# abbreviations, or tmux default-shell not set to fish. This catches silent
# partial installs like FM-NEW on a fresh machine.
#
# Flags (v2.1 — removal):
...
#                      — never a file without it), TPM, fleetmux-start, the
#                      fleetmux-managed fish config, and the shell-RC init
#                      lines it appended. Timestamped `.bak-*` backups from
#                      earlier runs are left in place
...
FISH_CONFIG_PREEXISTS=false
[ -f "$FISH_CONFIG_FILE" ] && FISH_CONFIG_PREEXISTS=true
...
# ── helper: write fleetmux-managed fish config ────────────────────────────────
# Called when --with-fish/--full is used. The file is only written if it does
# not already exist OR if it already carries the fleetmux sentinel (so repeated
# runs converge). A user-managed fish config without the sentinel is left alone.
_write_fish_config() {
  local fish_config_dir
  fish_config_dir="$(dirname "$FISH_CONFIG_FILE")"
  mkdir -p "$fish_config_dir"

  if [ -f "$FISH_CONFIG_FILE" ] && ! grep -qF "$FLEETMUX_SENTINEL" "$FISH_CONFIG_FILE" 2>/dev/null; then
    warn "Existing $FISH_CONFIG_FILE does not carry the fleetmux sentinel — left untouched."
    warn "Add the following manually if you want fleetmux fish wiring:"
    warn "  starship init fish | source"
    warn "  abbr -a gst git status"
    return 0
  fi

  cat > "$FISH_CONFIG_FILE" <<FISHCFG
# fleetmux-managed — generated by fleetmux installer
# This file is safe to customize, but keep the sentinel line above so re-runs
# can update it idempotently.

# Suppress the "Welcome to fish" greeting on every nested shell.
set -g fish_greeting

# Starship prompt.
starship init fish | source

# Agent-CLI PATH hygiene: make node-installed CLIs (claude, codex, cursor-agent,
# gemini) resolvable in tmux panes even when node was installed via nvm/volta/bun.
set -gx NVM_DIR "\$HOME/.nvm"
if test -s "\$NVM_DIR/nvm.sh"
  bash -c 'export NVM_DIR="\$NVM_DIR"; [ -s "\$NVM_DIR/nvm.sh" ] && . "\$NVM_DIR/nvm.sh" >/dev/null 2>&1; echo "\$PATH"' | read -gx PATH
end
fish_add_path "\$HOME/.local/bin"
fish_add_path "\$HOME/.bun/bin"
fish_add_path "\$HOME/.volta/bin"
set -l _fm_claude_app (printf '%s' "\$HOME/Library/Application Support/Claude/claude-code/"*/claude.app/Contents/MacOS)
if test -d "\$_fm_claude_app"
  fish_add_path "\$_fm_claude_app"
end
set -e _fm_claude_app
...
FISHCFG
  ...
  ok "Fish config installed: $FISH_CONFIG_FILE"
}

# ── helper: post-install verification ─────────────────────────────────────────
# Fails loudly if any wiring that the installer promised did not actually land.
# This is the guard against silent partial installs like FM-NEW.
_verify_fail() {
  printf '  %s✗%s %s\n' "$C_RED" "$C_RESET" "$*" >&2
}
_verify_install() {
  step "Verification — checking that everything wired up correctly"
  local failed=false

  if [ ! -f "$TMUX_CONF_FILE" ]; then
    _verify_fail "tmux config is missing: $TMUX_CONF_FILE"
    failed=true
  elif ! grep -qF "$FLEETMUX_SENTINEL" "$TMUX_CONF_FILE" 2>/dev/null; then
    _verify_fail "tmux config exists but is missing the fleetmux sentinel — not ours?"
    failed=true
  fi

  if [ ! -L "$TMUX_CONF_LINK" ]; then
    _verify_fail "tmux config symlink is missing: $TMUX_CONF_LINK -> $TMUX_CONF_FILE"
    failed=true
  fi

  if [ ! -f "$FMUX_START_DEST" ]; then
    _verify_fail "fleetmux-start is missing: $FMUX_START_DEST"
    failed=true
  fi

  if ! $OPT_MINIMAL && ! $OPT_NO_STARSHIP; then
    if [ ! -f "$STARSHIP_CONF" ]; then
      _verify_fail "Starship config is missing: $STARSHIP_CONF"
      failed=true
    elif ! grep -qF "$FLEETMUX_SENTINEL" "$STARSHIP_CONF" 2>/dev/null; then
      _verify_fail "Starship config exists but is missing the fleetmux sentinel."
      failed=true
    fi
  fi
  ...
  if $OPT_WITH_FISH; then
    if [ ! -f "$FISH_CONFIG_FILE" ]; then
      if $FISH_CONFIG_PREEXISTS; then
        warn "No fleetmux fish config was written because one existed before install."
        warn "Ensure your fish config contains: starship init fish | source"
        warn "Ensure your fish config contains: abbr -a gst git status"
      else
        _verify_fail "Fish config is missing: $FISH_CONFIG_FILE"
        failed=true
      fi
    else
      if ! grep -qF "set -g fish_greeting" "$FISH_CONFIG_FILE" 2>/dev/null; then
        _verify_fail "Fish greeting is not suppressed in $FISH_CONFIG_FILE"
        failed=true
      fi
      if ! grep -qF "starship init fish | source" "$FISH_CONFIG_FILE" 2>/dev/null; then
        _verify_fail "Starship is not initialized in $FISH_CONFIG_FILE"
        failed=true
      fi
      if ! grep -qF "abbr -a gst" "$FISH_CONFIG_FILE" 2>/dev/null; then
        _verify_fail "Git abbreviations are missing in $FISH_CONFIG_FILE (e.g. 'abbr -a gst git status')"
        failed=true
      fi
    fi

    if [ -f "$TMUX_CONF_FILE" ]; then
      if ! grep -qE '^\s*set\s+-g\s+default-shell' "$TMUX_CONF_FILE" 2>/dev/null; then
        _verify_fail "tmux default-shell is not set — panes may launch /bin/sh instead of fish."
        failed=true
      fi
    fi
  fi
  ...
  if $failed; then
    printf '\n  %s✗ fleetmux installed, but verification FAILED.%s\n' "$C_RED" "$C_RESET" >&2
    printf '     See the errors above. Fix them and re-run the installer.\n' >&2
    exit 1
  fi

  ok "All promised wiring is present."
}
...
if $OPT_UNINSTALL; then
  ...
  # Fish config: only the fleetmux-managed file; user-managed configs are left.
  if [ -f "$FISH_CONFIG_FILE" ] && grep -qF "$FLEETMUX_SENTINEL" "$FISH_CONFIG_FILE" 2>/dev/null; then
    rm -f "$FISH_CONFIG_FILE"
    ok "Removed: $FISH_CONFIG_FILE"
    # Remove the fish config directory too if it's now empty.
...
if $OPT_WITH_FISH; then
  ...
  # Configure fish: abbreviations, greeting suppression, starship, PATH hygiene.
  _write_fish_config

  # Tell tmux to spawn fish in every pane (not just the shell tmux inherited).
  if [ -f "$TMUX_CONF_FILE" ]; then
    if grep -qE '^\s*set\s+-g\s+default-shell' "$TMUX_CONF_FILE" 2>/dev/null; then
      # Update an existing default-shell line to the resolved fish path.
      TMUX_CONF_TMP="$(mktemp)"
      sed -E "s|^(\s*set\s+-g\s+default-shell\s+).*|\1$FISH_PATH|" "$TMUX_CONF_FILE" > "$TMUX_CONF_TMP"
      mv "$TMUX_CONF_TMP" "$TMUX_CONF_FILE"
      ok "Updated tmux default-shell: $FISH_PATH"
    else
      printf '\n# fleetmux-managed: use fish in every tmux pane.\nset -g default-shell %s\n' "$FISH_PATH" >> "$TMUX_CONF_FILE"
      ok "Set tmux default-shell: $FISH_PATH"
    fi
  fi
...
# ── step 11b: post-install verification (fail loudly on partial install) ─────
_verify_install
